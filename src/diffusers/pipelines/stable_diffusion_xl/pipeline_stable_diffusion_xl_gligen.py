...
class StableDiffusionXLGLIGENPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        def tokenize_captions(captions):
            tokens_one = self.tokenizer(captions, max_length=self.tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt")
            tokens_two = self.tokenizer_2(captions, max_length=self.tokenizer_2.model_max_length, padding="max_length", truncation=True, return_tensors="pt")
            return tokens_one.input_ids, tokens_two.input_ids
        ...
        if gligen_phrases is not None:
            ...
            if attention_type == 'gated':
                ...
                text_input_ids = text_inputs.input_ids.to(self.text_encoder_2.device)
                prompt_embeds_p = self.text_encoder_2(text_input_ids, output_hidden_states=True)
                pooled_prompt_embeds_p = prompt_embeds_p[0]
                text_embeddings = pooled_prompt_embeds_p
            elif attention_type == 'gatedbefore':
                ...
                for p in gligen_phrases:
                    # Important, insert the magic word
                    (
                        prompt_embeds_p,
                    ...
                    text_embeddings.append(prompt_embeds_p)
            ...
            if attention_type == 'gated':
                ...
                text_embeddings = text_embeddings.unsqueeze(0).expand(repeat_batch, -1, -1, -1).clone()
